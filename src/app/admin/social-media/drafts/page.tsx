import Link from 'next/link';
import { createSupabaseAdminClient } from '@/lib/supabase/admin';
import { DraftsList } from '@/components/admin/social-drafts-list';
import { BulkScheduleButton } from '@/components/admin/bulk-schedule-button';

export const dynamic = 'force-dynamic';

export default async function DraftsPage({ searchParams }: { searchParams: Promise<{ status?: string }> }) {
  const { status } = await searchParams;
  const filter = status ?? 'draft';
  const admin = createSupabaseAdminClient();
  const { data } = await admin
    .from('social_media_drafts')
    .select('*')
    .eq('status', filter)
    .order('created_at', { ascending: false })
    .limit(100);

  return (
    <main className="mx-auto max-w-5xl px-6 py-10">
      <div className="mb-6 flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold">Social drafts</h1>
          <p className="mt-1 text-sm text-zinc-500">Edit caption, image, schedule. Publishing pipeline lands in Session 10.</p>
        </div>
        <div className="flex gap-2">
          <Link href="/admin/social-media/candidates" className="text-sm text-blue-700 hover:underline">← Candidates</Link>
          <Link href="/admin" className="text-sm text-zinc-500 underline">← Admin</Link>
        </div>
      </div>

      <div className="mb-6">
        <BulkScheduleButton />
      </div>

      <div className="mb-4 flex gap-2 text-xs">
        {(['draft', 'scheduled', 'posted', 'failed'] as const).map((s) => (
          <Link key={s} href={`/admin/social-media/drafts?status=${s}`}
                className={`rounded-full px-3 py-1 ${filter === s ? 'bg-zinc-900 text-white dark:bg-white dark:text-zinc-900' : 'border border-zinc-300 dark:border-zinc-700'}`}>
            {s}
          </Link>
        ))}
      </div>

      <DraftsList drafts={data ?? []} />
    </main>
  );
}
