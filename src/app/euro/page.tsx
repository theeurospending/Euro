...
export default async function EuroPage() {
  ...
  const rateMarkers = data.events
    ...
    .map((e) => ({ date: e.event_date, label: e.title.length > 22 ? `${e.title.slice(0, 22)}…` : e.title, color: PALETTE.warning }));

  const hicpMarkers = data.events
    .filter((e) => e.category === 'crisis' || e.category === 'rate_change')
    .map((e) => ({ date: e.event_date, label: e.title.length > 22 ? `${e.title.slice(0, 22)}…` : e.title, color: PALETTE.negative }));
    ...
    <>
      ...
      <main className="mx-auto max-w-5xl px-6 py-10 text-[var(--brand-navy)]">
        ...
        <Section title="EUR / USD" hint="ECB reference exchange rate, daily">
          <Card>
            <TimeSeriesLine series={[{ label: 'EUR/USD', color: PALETTE.primary, data: data.eurUsd }]} format="plain" height={320} />
          </Card>
        </Section>
        ...
        <Section title="Sovereign yield divergence (PIIGS vs. Bund)" hint="10Y benchmark monthly — the chart that defined 2010-2012">
          <Card>
            <TimeSeriesLine series={yieldsSeries} format="pct1" height={400} />
          </Card>
